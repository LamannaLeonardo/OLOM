...
class PALAgent:
    ...
    def init_mdp(self):
        ...
        # Initialize the agent POMDP with a single propositional state, a single object with its state
        self.mdp.init_mdp(self.trace)

        # Initialize the set of objects
        self.objects = [Object(f"obj_{c}",
                               states=[self.mdp.clusters[f'c{k}']
                                       for k in range(self.mdp.obsk.shape[1])
                                       if self.mdp.obsk[:, k].sum() > 0])
                        for c in range(self.mdp.obsc.shape[0])]

    def prev_to_new_clusters(self, new_clusters):

        # Build a matrix M where M[i,j] indicates the number of observations of the previous i-th cluster that
        # belongs to the current j-th cluster
        M = pd.DataFrame(columns=sorted(list(set(new_clusters.values()))),
                         index=[f"{c}_prev" for c in sorted(list(set(self.prev_clusters.values())))])

        for prev_c in M.T.columns.values:
            prev_c = prev_c.replace('_prev', '', 1)
            prev_c_data = [d for d, c in self.prev_clusters.items() if c == prev_c]
            new_c_data = [len([d for d in prev_c_data if d in new_clusters.keys() and new_clusters[d] == new_c])
                          for new_c in M.columns.values]
            M.loc[prev_c + '_prev'] = new_c_data

        new_data = [len([d for d, c in new_clusters.items() if d not in self.prev_clusters.keys() and c == new_c])
                    for new_c in M.columns.values]
        M.loc['new'] = new_data
        new_obj_clusters = set(new_clusters.values())
        old_obj_clusters = {f"{v}_prev" for k, v in self.prev_clusters.items() if isinstance(k, str)}
        Mobj = M.filter(items=new_obj_clusters, axis=1).filter(items=old_obj_clusters, axis=0)
        Mobj = Mobj.div(Mobj.sum(axis=1), axis=0)
    ...
    def learn(self,
        ...
        self.logger.write(f"\n ############## CONFIGURATION ##############")
        self.logger.write(f"Random seed: {env.seed}")
        self.logger.write(f"Number of episode steps: {self.cfg['train']['learning_steps']}")
        self.logger.write(f"Trace length: {self.cfg['train']['hyperparameters']['history_len']}")
        self.logger.write(f"Number of environment objects: {len(env._state['objects'])}")
        self.logger.write(f"Actuation noise: {env.cfg['actuation_noise']}")
        self.logger.write(f" ###########################################\n")

        start = datetime.now()

        # Perceive initial state
        obs, _ = env.reset(options={"_state": env._state, "goal_states": env.goal_states}, seed=env_kwargs['seed'])
        env.action_space.seed(seed=env_kwargs['seed'])
        obs = self.discretize(obs)

        self.trace.observations.append(obs)
        self.mdp.init(obs)

        for i in range(self.cfg['train']['learning_steps']):

            # "Plan": choose a random action
            action_id = env.action_space.sample(mask=np.array([0, 1, 1, 1, 1], dtype=np.int8))  # do not sample `stop` action

            # Act and perceive
            obs, reward, terminated, truncated, info = env.step(action_id)

            # Create a new observation from the current perception
            obs = self.discretize(obs)
            self.trace.observations.append(obs)
            operator = env.operators[action_id]

            # TODO: manage objects
            action = Action(objects=operator.types, name=operator.name, types=operator.types)
            self.trace.actions.append(action)
            self.logger.write(f'{i}: {action}')

            # Update MDP from trace
            if (i + 1) % self.cfg['train']['hyperparameters']['history_len'] == 0:

                # Build a matrix KK' where KK'[i,j] indicates the probability of the new j-th cluster k'_j given
                # the previous i-th cluster k_i
                KK = np.eye(N=self.mdp.obsk.shape[1], M=len(self.mdp.discrete_obs), dtype=int)

                # Map the observation function from the previous cluster space K into the new cluster space K'
                self.mdp.to_new_clusters(KK)

                # Update the OPO-RMDP with the new trace
                self.mdp.update(self.trace)

                assert np.all([len(o.states) > 0 for o in self.mdp.objects])

                # Reset the current trace so that it contains only the current observation
                self.trace.reset()
        ...
        # Plot transition matrices for unary operators
        fig, axes = plt.subplots(1, len(self.mdp.Tv), figsize=(12, 6))  # 1 row, 2 columns
        for op in range(len(self.mdp.Tv)):
            sn.heatmap(self.mdp.Tv[op],
                       vmin=0, vmax=1, annot=True, fmt=".2f",
                       # cbar=True,
                       xticklabels=[f"$s_v^{{{i}}}$" for i in range(self.mdp.Tv[op].shape[1])],
                       yticklabels=[f"$s_v^{{{i}}}$" for i in range(self.mdp.Tv[op].shape[0])],
                       ax=axes[op])
            axes[op].set_title(self.mdp.ops_unary[op])

        plt.tight_layout()
        plt.savefig(f'{self.logger.log_dir}/Tv.png')
        plt.close()

        # Plot transition matrices for nullary operators
        fig, axes = plt.subplots(1, len(self.mdp.ops_zeroary), figsize=(12, 6))  # 1 row, 2 columns
        # for op in range(1, len(self.mdp.To)):
        for op in range(len(self.mdp.To)):
            sn.heatmap(self.mdp.To[op],
                       vmin=0, vmax=1, annot=True, fmt=".2f",
                       # cbar=True,
                       xticklabels=[f"$s_{{\emptyset}}^{{{i}}}$" for i in range(self.mdp.To[op].shape[1])],
                       yticklabels=[f"$s_{{\emptyset}}^{{{i}}}$" for i in range(self.mdp.To[op].shape[0])],
                       ax=axes[op])
            axes[op].set_title(self.mdp.ops_zeroary[op])
        plt.tight_layout()
        plt.savefig(f'{self.logger.log_dir}/To.png')
        plt.close()

        # Plot observation function
        sn.heatmap(self.mdp.obsk,
                   vmin=0, vmax=1, annot=True, fmt=".2f",
                   xticklabels=[f"$k^{{{i}}}$" for i in range(self.mdp.obsk.shape[1])],
                   yticklabels=[f"$s_{{v}}^{{{i}}}$" for i in range(self.mdp.obsk.shape[0])]
                   )
        plt.title('$obs_k$')
        plt.tight_layout()
        plt.savefig(f'{self.logger.log_dir}/obsk.png')
        plt.close()
        ...
        metrics = pd.DataFrame([{
            'Number of parallel environments': 1,  # TODO: parallelize pal agent
            'Learning steps': self.cfg['train']['learning_steps'],
            'Time seconds': (datetime.now() - start).seconds,
            'Memory usage (MB)': peak / 10**6,
            'Actuation noise': env.cfg['actuation_noise'],
            'Number of environment objects': len(env._state['objects'])
        }])
    ...
    def solve(self,
        ...
        obs, _ = env.reset(options={'_state': env._state, 'goal_states': env.goal_states}, seed=seed)
        ...
        if current_cluster is None or np.any([g is None for g in goal_states]):
            ...
            for i in range(max_steps):
                ...
                if self.cfg['planner'] == 'POUCT' and action.name != 'stop':
                    ...
                    self.mdp.update(self.trace, update_transf=True)
                    self.trace.reset()
